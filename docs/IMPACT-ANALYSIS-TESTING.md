...
## Issue #31 后续回归（0.1.13）

- table-driven 覆盖 `oxlint-disable` / `oxlint-enable` 切换、规则列表变化、增删与跨声明移动。
- 每种变化同时覆盖 target 自身文件和 dependency 文件，断言保守影响、无 `semantic-noop`。
- 未变化的 Oxlint 范围包装分别覆盖符号级与类型精化，防止修复退化为永久文件级回退。
- 从公开 npm 全新安装 `0.1.13`，复验 Issue #31 合成用例的摘要与 JSON 报告。
