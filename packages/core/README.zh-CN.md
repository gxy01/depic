...
从 `0.1.10` 起（Issue #25），未改变的顶层指令包装按声明边界归属，而不是绝对代码
...
从 `0.1.15` 起（Issue #34），每个截断 target 都单独产生 `chain-limit-reached`
诊断，其结构化 `chainLimit` 包含已返回/已知至少链数、当前 target/报告总限制、限制来源、
